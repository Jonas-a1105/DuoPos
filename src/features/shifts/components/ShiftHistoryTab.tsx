...
export default function ShiftHistoryTab({ shiftHistory, onOpenReport }: ShiftHistoryTabProps) {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      {shiftStatsAnalysis && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-left">
          ...
          <div className="bg-white border-2 border-gray-200 border-b-[6px] rounded-3xl p-5 flex flex-col justify-between h-32">
            ...
            <div>
              <span className="text-2xl font-black text-[#58cc02] font-mono">
                ${shiftStatsAnalysis.totalVolume.toFixed(2)}
              </span>
              <p className="text-[8px] text-gray-400 font-bold uppercase block mt-1 leading-none">
                Volumen acumulado arqueado
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-gray-200 border-b-[6px] rounded-3xl p-5 flex flex-col justify-between h-32">
            <span className="text-[10px] uppercase font-black tracking-wider text-gray-400">
              Venta Promedio por Turno
            </span>
            <div>
              <span className="text-2xl font-black text-[#1cb0f6] font-mono">
                ${shiftStatsAnalysis.avgSales.toFixed(2)}
              </span>
              <p className="text-[8px] text-gray-400 font-bold uppercase block mt-1 leading-none">
                Rendimiento ponderado de caja
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-gray-200 border-b-[6px] rounded-3xl p-5 flex flex-col justify-between h-32">
            ...
            <div>
              <span className="text-2xl font-black text-amber-505 font-mono">{shiftStatsAnalysis.pctBalanced}%</span>
              <p className="text-[8px] text-gray-400 font-bold uppercase block mt-1 leading-none">
      ...
      <div className="bg-white border-2 border-gray-200 border-b-[6px] rounded-3xl p-5 md:p-6 space-y-4 text-left">
        <div className="border-b pb-3 flex justify-between items-center">
          <div>
            <h3 className="text-lg font-black text-gray-800 uppercase">🏆 Registro Histórico de Turnos y Auditorías</h3>
            <p className="text-xs text-gray-450 font-bold mt-0.5">
              Control vinculante del flujo de efectivo recaudado históricamente
            </p>
          </div>
        ...
        {shiftHistory.length === 0 ? (
          <div className="py-16 text-center text-gray-400 space-y-3">
            ...
            <p className="text-sm font-black uppercase text-gray-500">
              No hay turnos cerrados registrados para auditar
            </p>
            <p className="text-xs text-gray-400 max-w-sm mx-auto font-medium leading-relaxed">
              Una vez que abras tu primer turno de caja y realices el posterior cierre y arqueo de caja, los informes de
              auditoría aparecerán protegidos aquí.
            </p>
          </div>
        ) : (
                  ...
                  return (
                    <tr key={hist.id} className="hover:bg-gray-50 transition-colors">
                      <td className="py-3">
                        ...
                        <span className="text-[10px] font-mono text-gray-400 font-bold block mt-1 leading-none">
                          {hist.id}
                        </span>
                      </td>
                      <td className="py-3">
                        ...
                        {hist.closingTime && (
                          <span className="text-[9px] text-[#9c9c9c] block mt-1 leading-none font-medium">
                            Cerrado:{' '}
                            {new Date(hist.closingTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </span>
                        )}
                      </td>
                      <td className="py-3 text-center">
                        <span className="text-gray-800 font-bold font-mono">${hist.salesVolume.toFixed(2)}</span>
                        <span className="text-[9px] text-gray-400 block font-normal leading-none mt-1">
                          {hist.salesCount} tickets
                        </span>
                      </td>
                      ...
                      <td
                        className={`py-3 text-right font-mono font-black ${isDescuadre ? 'text-red-500' : 'text-[#3c9e01]'}`}
                      >
                        {(hist.difference || 0) >= 0 ? '+' : ''}
                        {(hist.difference || 0).toFixed(2)}
                      </td>
                      <td className="py-3 text-right">
