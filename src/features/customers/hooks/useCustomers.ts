...
import { syncInsert, syncSave, syncDelete, generateUUID, syncDailyStats } from '../../../database/supabaseSync';
...
import { emitCustomerCreated, emitCustomerUpdated, emitCustomerDeleted, emitLoyaltyPointsChanged } from '../customers.events';
...
export function useCustomers() {
  ...
  const addCustomer = useCallback(
    async (
      ...
      emitCustomerCreated({ customerId: formatted.id, customerName: formatted.name });
      addAuditLog(
        'clientes',
        'crear',
  ...
  const updateCustomer = useCallback(
    async (cust: Customer) => {
      ...
      if (changedItem) {
        ...
        emitCustomerUpdated({
          customerId: changedItem.id,
          customerName: changedItem.name,
          changes: { name: changedItem.name, email: changedItem.email, phone: changedItem.phone, creditLimit: changedItem.creditLimit },
        });
      }
  ...
  const deleteCustomer = useCallback(
    async (id: string) => {
      ...
      emitCustomerDeleted({ customerId: id, customerName: deletedName });
  ...
  const processCustomerLoyalty = useCallback(
    async (txn: {
      ...
      if (changedCust) {
        ...
        const prevCust = customers.find((c) => c.id === txn.customerId);
        if (prevCust && prevCust.gems !== changedCust.gems) {
          emitLoyaltyPointsChanged({
            customerId: changedCust.id,
            customerName: changedCust.name,
            previousGems: prevCust.gems,
            newGems: changedCust.gems,
            difference: changedCust.gems - prevCust.gems,
          });
        }
      }
    },
