...
export default function AdvancedAnalyticsTab({ transactions, products, user }: AdvancedAnalyticsTabProps) {
  ...
  const taxConfig = useMemo(() => {
    try {
      return JSON.parse(
        localStorage.getItem('duo_pos_tax_settings') || '{"taxName": "IVA", "taxRate": 16, "isTaxIncluded": true}',
      );
    } catch {
  ...
  const advancedMetrics = useMemo(() => {
    ...
    const productStats: Record<
      string,
      { id: string; name: string; emoji: string; qty: number; revenue: number; profit: number }
    > = {};
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
        ...
        <div className="bg-white border-2 border-[#e5e5e5] border-b-[6px] rounded-3xl p-5 flex flex-col justify-between min-h-36">
          <div className="flex justify-between items-center text-xs font-black text-gray-400 uppercase tracking-wide">
            <span>
              Total {taxConfig.taxName} ({taxConfig.taxRate}%) Recaudado
            </span>
          ...
          <p className="text-[10px] text-gray-455 font-bold mt-1 uppercase">Débito fiscal por liquidar</p>
        </div>
        ...
        <div className="bg-white border-2 border-[#e5e5e5] border-b-[6px] rounded-3xl p-5 flex flex-col justify-between min-h-36 font-sans">
          ...
          <p className="text-[10px] text-gray-455 font-bold mt-1 uppercase">Retorno de inversión promedio</p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          ...
          <div className="flex gap-4 text-[10px] font-black uppercase text-gray-500">
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-[#58cc02]" /> Ventas
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-blue-400" /> Costos
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-[#e5e5e5] border-b-[6px] rounded-3xl p-5 md:p-6 space-y-4 lg:col-span-1">
          ...
          <div className="space-y-4 pt-1">
            {(() => {
              ...
              const totalSum =
                (Object.values(advancedMetrics.categorySales).reduce((a, b) => Number(a) + Number(b), 0) as number) ||
                1;

              if (categoriesList.length === 0) {
                return (
                  <p className="text-xs text-gray-400 font-bold py-6 text-center">No hay datos registrados aún.</p>
                );
              }
                ...
                return (
                  <div key={cat} className="space-y-1.5">
                    ...
                    <div className="w-full bg-gray-150 h-2 rounded-full overflow-hidden">
                      <div className="bg-[#1cb0f6] h-full rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                );
              });
                ...
                <tbody className="divide-y divide-gray-100 font-sans">
                  {advancedMetrics.productsLeaderboard.slice(0, 5).map((p, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="py-2.5 text-[#58cc02] text-right font-black font-mono">+${p.profit.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="bg-amber-50 border-2 border-amber-250 border-b-[6px] rounded-3xl p-5 md:p-6 shadow-xs text-left">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1.5 flex-1">
            ...
            <p className="text-xs text-amber-900 leading-relaxed max-w-xl">
              Averigua cuántas transacciones generales necesitas concretar para alcanzar una meta mensual de ingresos
              limpios de fábrica según los tickets promedio ponderados.
            </p>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-amber-200 flex flex-col sm:flex-row items-center gap-4 text-xs text-amber-900">
          ...
          <p className="font-extrabold flex-1 text-center sm:text-left leading-relaxed">
            {(() => {
              const averageProfitPerItem =
                advancedMetrics.totalItemsCount > 0 ? totalProfitAllTime / advancedMetrics.totalItemsCount : 1.5;

              const unitsNeeded = Math.ceil(monthlyTargetProfit / Math.max(averageProfitPerItem, 0.5));
              const averageCartItems =
                transactions.length > 0 ? advancedMetrics.totalItemsCount / transactions.length : 2.1;
