...
export default function CriticalAlerts({ products, onUpdateProduct, onGrantXp }: CriticalAlertsProps) {
    ...
    <div className="space-y-4 animate-fadeIn text-left">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border-2 border-gray-250 border-b-[6px] rounded-3xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] font-black uppercase text-gray-400 tracking-wide mt-1 block">
              Insumos Deficitarios
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-gray-250 border-b-[6px] rounded-3xl p-4 flex items-center gap-3">
          ...
          <div>
            <span className="text-2xl font-black text-amber-600 block leading-none">{totalQtyGap} un</span>
            <span className="text-[10px] font-black uppercase text-gray-400 tracking-wide mt-1 block">
              Brecha a Stock Ideal
            </span>
          </div>
        </div>
      ...
      <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-5 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="space-y-0.5 text-left flex-1">
          <span className="text-[9px] bg-amber-200 text-amber-900 px-2 py-0.5 rounded font-black uppercase">
            Fórmula de Reorden Automática
          </span>
          ...
          <p className="text-xs text-amber-800 font-bold max-w-lg leading-relaxed">
            Reabastece de golpe todas las existencias críticas hasta el tope ideal de{' '}
            <strong className="text-amber-950">{defaultIdealStock} unidades</strong> por producto. Otorga +50 XP.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <div className="flex flex-col items-center bg-white p-2 border border-amber-200 rounded-2xl font-bold">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              <button
                ...
                onClick={() => {
                  playSound('click');
                  setDefaultIdealStock((p) => Math.max(10, p - 5));
                }}
              ...
              <button
                ...
                onClick={() => {
                  playSound('click');
                  setDefaultIdealStock((p) => Math.min(100, p + 5));
                }}
                className="w-6 h-6 bg-slate-100 hover:bg-slate-200 rounded text-center font-black text-xs cursor-pointer select-none"
              >
                +
          ...
          {criticalProducts.length > 0 ? (
            <button
              ...
              onClick={() => {
                ...
                const totalCost = criticalProducts.reduce(
                  (sum, p) => sum + Math.max(0, defaultIdealStock - p.stock) * p.cost,
                  0,
                );
                ...
                playSound('success');
                alert(
                  `🚚 Lote L1 Surtido: Se cargaron ${totalQty} un. para ${criticalProducts.length} productos. Costo: $${totalCost.toFixed(2)}. (+50 XP Logística)`,
                );
              }}
            ...
            <span className="text-xs font-black text-green-600 bg-green-55 border border-green-200 px-4 py-3 rounded-2xl select-none">
              BODEGA CRÍTICA AL 100% ✅
            </span>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-gray-205 border-b-[6px] rounded-3xl p-5 space-y-4">
        <h4 className="text-xs font-black uppercase text-gray-400 border-b pb-2">
          Artículos Bajo el Mínimo Autorizado ({criticalProducts.length})
        </h4>
        {criticalProducts.length === 0 ? (
          <p className="text-center py-6 text-xs text-gray-400 font-bold">
            🎉 Todos los productos cumplen el margen mínimo de reserva.
          </p>
        ) : (
          <div className="divide-y divide-gray-100 text-xs">
            {criticalProducts.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 first:pt-0"
                >
                  <div className="flex items-center gap-2">
                    <span className="text-2xl select-none">{p.emoji || '📦'}</span>
                    <div>
                      <p className="font-extrabold text-gray-850 text-sm">{p.name}</p>
                      <p className="text-gray-450 text-[10px] font-bold">
                        Stock actual: <strong className="text-red-500">{p.stock} un</strong> / Ideal:{' '}
                        {defaultIdealStock}
                      </p>
                    </div>
                  </div>
