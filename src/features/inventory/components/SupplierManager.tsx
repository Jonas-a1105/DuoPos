      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-indigo-55 border-2 border-indigo-200 rounded-3xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] uppercase font-black text-indigo-500 leading-none mt-1.5 block">Aliados Frecuentes</span>
          </div>
        </div>
        ...
        <div className="bg-emerald-50 border-2 border-emerald-200 rounded-3xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] uppercase font-black text-emerald-600 leading-none mt-1.5 block">Fiabilidad Media</span>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] uppercase font-black text-amber-600 leading-none mt-1.5 block">Deudas Pendientes AP</span>
          </div>
        </div>
      </div>
          ...
          <div key={sup.id} className="bg-white border-2 border-gray-200 border-b-[6px] rounded-3xl p-4.5 flex flex-col justify-between space-y-3 relative">
            <div className="space-y-2 text-left">
              ...
              <div className="space-y-1 text-xs">
                <p className="font-semibold text-gray-650 flex items-center gap-1"><span>👤 Contacto:</span> <strong className="text-gray-850 font-extrabold">{sup.contact}</strong></p>
            ...
            <div className="pt-2 border-t border-dashed border-gray-200 space-y-2">
              <div className="flex justify-between items-center text-[10px] font-black">
                ...
                <span className={sup.reliability >= 95 ? 'text-green-600' : sup.reliability >= 90 ? 'text-indigo-650' : 'text-red-500'}>
                  {sup.reliability}%
                </span>
              </div>
              ...
              <div className="flex justify-between items-center bg-gray-50 p-2 rounded-xl text-left border">
                ...
                {sup.balance > 0 ? (
                  <button 
                    onClick={() => { onSelectSupplierForPayout(sup.id); onSelectTab('accounts'); playSound('click'); }} 
                  ...
                  <span className="text-[9px] text-green-600 bg-green-50 px-2 py-1 rounded-lg border border-green-250 font-black select-none">Al día</span>
                )}
              </div>
            </div>
          </div>
      ...
      {isSupplierFormOpen && (
        <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-fadeIn text-gray-800">
          <div className="bg-white border-2 border-gray-200 border-b-8 rounded-3xl max-w-lg w-full p-6 space-y-4 relative text-left">
            <button onClick={() => setIsSupplierFormOpen(false)} className="absolute right-4 top-4 p-2 text-gray-400 hover:text-gray-650 rounded-full"><X size={20} /></button>
            ...
            <form onSubmit={handleSaveSupplier} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs pt-2">
                <button type="button" onClick={() => setIsSupplierFormOpen(false)} className="bg-slate-100 hover:bg-slate-205 py-2.5 font-bold rounded-xl border">Cancelar</button>
