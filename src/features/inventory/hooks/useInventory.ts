...
import { syncInsert, syncSave, syncDelete, generateUUID, syncSavePurchaseOrder } from '../../../database/supabaseSync';
import { toast } from '../../../shared/ui/FlashNotifications/FlashNotifications';
import { emitProductCreated, emitProductUpdated, emitProductDeleted, emitStockChanged } from '../inventory.events';
...
export function useInventory() {
  ...
  const addProduct = useCallback(
    async (newProd: Omit<Product, 'id'>, activeBranchId: string, onXp?: (amount: number) => void) => {
      ...
      emitProductCreated({ productId: formatted.id, productName: formatted.name, category: formatted.category });
  ...
  const updateProduct = useCallback(
    async (prod: Product, activeBranchId: string) => {
      ...
      if (changedItem) {
        ...
        emitProductUpdated({
          productId: changedItem.id,
          productName: changedItem.name,
          changes: { name: changedItem.name, price: changedItem.price, stock: changedItem.stock, category: changedItem.category },
        });
      }
  ...
  const deleteProduct = useCallback(
    async (id: string) => {
      ...
      emitProductDeleted({ productId: id, productName: deletedName });
  ...
  const decreaseStock = useCallback(
    async (productId: string, qty: number, activeBranchId: string) => {
      ...
      if (changed) {
        await syncSave<Product>('products', 'duo_pos_products', updated, changed);
        emitStockChanged({
          productId: changed.id,
          productName: changed.name,
          previousStock: (products.find((p) => p.id === productId)?.stock ?? 0) + qty,
          newStock: changed.stock,
          quantity: qty,
        });
      }
    },
