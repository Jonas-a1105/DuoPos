...
const ShieldCrest: React.FC<ShieldCrestProps> = ({ level = 1, rank, size = 80, animate = true, className = '' }) => {
  ...
  return (
    <div className={wrapperClass} style={{ width: size, height: size * (140 / 120), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
      <svg viewBox={vb} width={size} height={size * (140 / 120)} xmlns="http://www.w3.org/2000/svg" style={{ overflow: 'visible' }}>
        <defs>
          <linearGradient id="bronzeShieldGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#cd7f32" /><stop offset="50%" stopColor="#a0522d" /><stop offset="100%" stopColor="#8b4513" />
          </linearGradient>
          <linearGradient id="silverShieldGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#f1f5f9" /><stop offset="35%" stopColor="#cbd5e1" /><stop offset="70%" stopColor="#94a3b8" /><stop offset="100%" stopColor="#64748b" />
          </linearGradient>
          <linearGradient id="goldShieldGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#fbbf24" /><stop offset="35%" stopColor="#f59e0b" /><stop offset="70%" stopColor="#d97706" /><stop offset="100%" stopColor="#b45309" />
          </linearGradient>
        ...
        {computedRank === 'master' && (
          <g className="shield-rotate-ring" opacity="0.8">
            ...
            <circle cx="114" cy="70" r="3" fill="#10b981" /><circle cx="6" cy="70" r="3" fill="#8b5cf6" />
          </g>
        )}
        {computedRank !== 'bronze' && (
          <g opacity="0.9">
            <path d="M20 40 Q5 32 2 52 Q12 56 22 52" fill={computedRank === 'silver' ? '#cbd5e1' : computedRank === 'gold' ? '#fbbf24' : '#10b981'} stroke="#1e293b" strokeWidth="1" />
            <path d="M19 50 Q0 45 -2 65 Q10 68 20 62" fill={computedRank === 'silver' ? '#94a3b8' : computedRank === 'gold' ? '#f59e0b' : '#059669'} stroke="#1e293b" strokeWidth="1" />
            <path d="M100 40 Q115 32 118 52 Q108 56 98 52" fill={computedRank === 'silver' ? '#cbd5e1' : computedRank === 'gold' ? '#fbbf24' : '#10b981'} stroke="#1e293b" strokeWidth="1" />
            <path d="M101 50 Q120 45 122 65 Q110 68 100 62" fill={computedRank === 'silver' ? '#94a3b8' : computedRank === 'gold' ? '#f59e0b' : '#059669'} stroke="#1e293b" strokeWidth="1" />
          </g>
        )}
        {computedRank === 'master' && (
          <g transform="translate(42, -5)">
            ...
            <circle cx="8" cy="2" r="1.5" fill="#ef4444" /><circle cx="18" cy="10" r="1.2" fill="#3b82f6" /><circle cx="28" cy="2" r="1.5" fill="#ef4444" />
          </g>
        )}
        <path d="M60 21 Q91 21 96 50 Q96 82 60 114 Q24 82 24 50 Q29 21 60 21 Z" fill="rgba(0,0,0,0.15)" />
        <path d="M60 15 Q95 15 100 48 Q100 85 60 120 Q20 85 20 48 Q25 15 60 15 Z" fill={computedRank === 'bronze' ? 'url(#bronzeShieldGrad)' : computedRank === 'silver' ? 'url(#silverShieldGrad)' : computedRank === 'gold' ? 'url(#goldShieldGrad)' : 'url(#masterShieldGrad)'} stroke={computedRank === 'bronze' ? '#8b4513' : computedRank === 'silver' ? '#475569' : computedRank === 'gold' ? '#d97706' : '#047857'} strokeWidth="3.5" strokeLinejoin="round" />
        <path d="M60 21 Q90 21 94 48 Q94 79 60 111 Q26 79 26 48 Q30 21 60 21 Z" fill="none" stroke={computedRank === 'bronze' ? '#ffd8a8' : computedRank === 'silver' ? '#f8fafc' : computedRank === 'gold' ? '#fef08a' : '#a7f3d0'} strokeWidth="1.2" opacity="0.65" />
        {(computedRank === 'gold' || computedRank === 'master') && (
          <g>
            <circle cx="28" cy="48" r="3" fill="#ef4444" stroke="#7f1d1d" strokeWidth="0.8" /><circle cx="27" cy="47" r="0.8" fill="white" opacity="0.6" />
            <circle cx="92" cy="48" r="3" fill="#ef4444" stroke="#7f1d1d" strokeWidth="0.8" /><circle cx="91" cy="47" r="0.8" fill="white" opacity="0.6" />
        ...
        <g>
          ...
          <polygon points="78,38 72,42 76,46" fill="#ef4444" /><polygon points="78,38 74,34 70,38" fill="#ef4444" />
          <text x="60" y="101" textAnchor="middle" fontSize="10" fontWeight="900" fill="white" stroke="#000" strokeWidth="0.5" letterSpacing="0.5">Lvl {level}</text>
        </g>
        <g clipPath="url(#shieldClip)">
          <rect className="shield-sheen-sweep" x="-40" y="15" width="40" height="110" fill="url(#sheenGrad)" transform="skewX(-30)" opacity="0.7" />
        </g>
      </svg>
    </div>
  );
