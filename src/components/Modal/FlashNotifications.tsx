...
export function FlashNotifications() {
  ...
  useEffect(() => {
    const handleAdd = (notification: FlashNotification) => {
      setNotifications((prev) => {
        ...
        // Limit the active notifications to a maximum of 3 to avoid vertical stacking overflow
  ...
  return (
    <div className="fixed top-4 right-4 z-[9999] pointer-events-none w-full max-w-sm flex flex-col gap-2.5 px-4 sm:px-0">
      <AnimatePresence>
        {notifications.map((n) => (
...
function NotificationCard({
  ...
  // Color mappings matching premium, clean, and brand-consistent design
  const config = {
    success: {
      bg: 'bg-[#eefcf2]/95 backdrop-blur-md',
      border: 'border-[#58cc02] border-b-[3.5px]',
      text: 'text-[#1c7b01]',
    ...
    error: {
      ...
      border: 'border-[#ff4b4b] border-b-[3.5px]',
      text: 'text-[#bd1616]',
    ...
    warning: {
      ...
      border: 'border-[#ff9600] border-b-[3.5px]',
      text: 'text-[#a66200]',
    ...
    info: {
      ...
      border: 'border-[#1cb0f6] border-b-[3.5px]',
      text: 'text-[#005c8a]',
    ...
    achievement: {
      ...
      border: 'border-[#8e44ad] border-b-[3.5px]',
      text: 'text-[#5a008a]',
  ...
  return (
    <motion.div
      ...
      className={`pointer-events-auto w-full select-none rounded-xl p-3 flex gap-2.5 shadow-lg shadow-black/5 border border-[#e5e5e5]/80 ${config.bg} ${config.border} relative overflow-hidden`}
    >
      <div className="flex-shrink-0 flex items-center justify-center text-lg w-8 h-8 bg-white/80 rounded-full border border-gray-150 relative">
        <span className="absolute text-[11px] top-[-5px] right-[-5px]">{config.emoji}</span>
        {config.icon}
      </div>
      ...
      <button
        ...
        className="text-gray-400 hover:text-gray-600 transition-colors h-fit p-0.5 rounded-md hover:bg-black/5 flex-shrink-0"
      >
        <X size={13} />
      </button>
