    ...
    <div className="fixed inset-0 z-50 bg-[#141414]/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white border-2 border-[#e5e5e5] border-b-[8px] rounded-3xl p-6 text-center space-y-4 animate-scaleUp">
        ...
        <p className="text-sm font-bold text-gray-500">
          Para entrar a la pestaña de{' '}
          <strong className="text-gray-800 font-extrabold">"{roleLockWarning.tabName}"</strong> necesitas rol de{' '}
          <span className="bg-amber-100 text-amber-700 px-2.5 py-1 rounded-lg border border-amber-200 uppercase text-xs font-black">
            {roleLockWarning.requiredRole}
          </span>
          .
        </p>
        <p className="text-xs text-gray-400 font-bold">
          Tu rol actual es:{' '}
          <span className="uppercase text-slate-600 underline font-black">
            {user.role === 'cashier' ? 'Cajero 💵' : user.role === 'supervisor' ? 'Supervisor ⚡' : 'Administrador 👑'}
          </span>
        </p>
          ...
          <>
            <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl text-left space-y-2 mt-4">
              <span className="text-[11px] font-black text-blue-600 uppercase tracking-widest block">
                🔧 Modo Demostración (Simulador de Permisos)
              </span>
              <p className="text-xs text-blue-700 leading-relaxed font-semibold">
                ¿Deseas verificar esta vista? Haz clic abajo para autodesignarte un nivel de acceso superior temporal en
                este navegador.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              <button
                onClick={() => {
                  ...
                  setActiveTab(
                    roleLockWarning.tabName === 'Configuración'
                      ? 'settings'
                      : roleLockWarning.tabName === 'Catálogos'
                        ? 'inventory'
                        : roleLockWarning.tabName === 'Logística'
                          ? 'logistics'
                          : 'dashboard',
                  );
